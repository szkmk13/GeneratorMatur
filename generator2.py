...
def wypisanie_liczb():
    ...
    symetria = choice(['osi OY', 'osi OX', 'początku układu współrzędnych'])
    f.write(f'b) wykres fukncji f(x)= {a}x\u00b2{choice(["+","-"])}{randint(2,5)}x przekształcimy przez symetrię względem {symetria}\n\n ')
    f.write(f"2a. Narysuj wykres funkcji f(x)= -(|x|{randint(1,4)})\u00b2. Wyznacz miejsca zerowe oraz podaj jej przedziały monotoniczności.\n\n")
    f.write(
    ...
    f.write(f"3. Narysuj wykres funkcji f(x)=|{randint(2,5)}-|x{choice(['+','-'])}{randint(2, 5)}|| i podaj jej zbiór wartości\n\n")
